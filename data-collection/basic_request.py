...
def get_airbnb_listing_info(client_id,**kwargs):
    listing_info_url = "https://api.airbnb.com/v2/listings/" + str(kwargs["listing_id"]) + "?client_id=" + client_id + "&locale=en-US&currency=USD&_format=v1_legacy_for_p3"
    r = requests.get(listing_info_url)
    return json.loads(r.text)["listing"]
